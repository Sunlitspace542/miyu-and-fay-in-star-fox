...
; Optimizations/fixes
; 0 = off, 1 = on
!opt_enginesnd = 1	; optimize engine sound generation code (requires patches to PSGSND2, SGSOUND2, and SGSOUND3)
					; saves 19 bytes
!opt_f1_f9 = 1		; remove unnecessary broken delay operand from $F1/$F9 VCMDs
					; saves 361 bytes
!opt_misc = 1		; various small optimizations
...
sfx_chain_table:
	...
	db $00, $00, $79, $BB, $00, $00, $00, $00 ; $B9 - $C0
	db $00, $00, $00, $00, $00                ; $C1 - $C5
...
sfx_parameters:
	...
	db $36, $34, $25, $35, $0F, $2E, $2F, $39 ; $B9 - $C0
	db $39, $39, $39, $39, $39                ; $C1 - $C5
...
sfx_ptrs:
	...
	dw se_miyuradiochat				; $C0 MIYU RADIO CHAT
	dw se_miyuradiochathit			; $C1 MIYU RADIO CHAT HIT
	dw se_miyuradiochatdown			; $C2 MIYU RADIO CHAT DOWN
	dw se_fayradiochat				; $C3 FAY RADIO CHAT
	dw se_fayradiochathit			; $C4 FAY RADIO CHAT HIT
	dw se_fayradiochatdown			; $C5 FAY RADIO CHAT DOWN
...
se_miyuradiochat: ; Miyu radio chat
	%teammateradiochat()
	%inst(1F) ; 1-up F
	%dur($12) : %vol($7D) : %pitch($9B)
	%pitch($95)
	%dur($0A) : %pitch($98)
	%pitch($99)
	%pitch($98)
...
se_miyuradiochathit: ; Miyu radio chat hit
	%teammateradiochat()
	%inst(1F) ; 1-up F
	%dur($0A) : %vol($7D) : %pitch($98)
	%dur($06) : %pitch($9A)
	%dur($14) : %pitch($97)
	%dur($06) : %pitch($9A)
	%dur($14) : %pitch($97)
...
se_miyuradiochatdown: ; Miyu radio chat down
	%downradiochat()
	%inst(1F) ; 1-up F
	%dur($1B) : %vol($7D) : %pitch($A0)
	%pitch($A0)
	%dur($14) : %pitch($97)
	%pitch($97)
	%pitch($97)
...
se_fayradiochatdown: ; Fay radio chat down
	%downradiochat()
	%inst(1F) ; 1-up F
	%dur($0A) : %vol($7D) : %pitch($9D)
	%pitch($98)
	%pitch($9F)
	%dur($12) : %pitch($97)
	%pitch($9D)
